...
type LoadingCategory interface {
	Insert(loadingCategory LoadingCategoryRow) error
	DeleteAll() error
}
...
func (lc PGXLoadingCategory) Insert(loadingCategory LoadingCategoryRow) error {
	lc.log.Debug("inserting LoadingCategoryRow", slog.String("loading_category_code", loadingCategory.LoadingCategoryCode))
	return database.InsertIntoTable(lc.ctx, lc.tx, "loading_categories", loadingCategory)
}
