...
CREATE TABLE IF NOT EXISTS darwin.messages (
				...
				,first_received_at timestamp WITH TIME ZONE NOT NULL -- The first time that railreader received this message.
				,last_received_at timestamp WITH TIME ZONE NOT NULL -- The most recent time that railreader has received this message.
...
CREATE TABLE IF NOT EXISTS darwin.alarms ( -- Alarms raised when Darwin has not received data from some of its sources.
				id uuid PRIMARY KEY
...
CREATE TABLE IF NOT EXISTS darwin.forecast_locations (
				...
				,platform_is_suppressed boolean NULL
...
CREATE TABLE IF NOT EXISTS darwin.formation_coaches (
				id uuid PRIMARY KEY

				,formation_uuid uuid NOT NULL
				,CONSTRAINT fk_formation_uuid FOREIGN KEY(formation_uuid) REFERENCES formations(id) ON DELETE CASCADE
