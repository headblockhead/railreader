...
func (c IngestCommand) newDarwin(log *slog.Logger, dbpool *pgxpool.Pool) (messageFetcherCommitter, messageHandler, error) {
	...
	darwinMessageHandler := darwin.NewMessageHandler(messageHandlerContext, log.With(slog.String("source", "darwin.handler")), dbpool, darwinFileGetter)
...
func loadNewestDarwinFiles(log *slog.Logger, dbpool *pgxpool.Pool, fg filegetter.FileGetter) error {
	...
	u, err := interpreter.NewUnitOfWork(context.Background(), log, dbpool, fg, nil, nil) // timetableID is set with interpreter.InterpretTimetable
	if err != nil {
